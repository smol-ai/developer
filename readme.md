...
```bash
python main.py --prompt "a Chrome extension that, when clicked, opens a small window with a page where you can enter a prompt for reading the currently open page and generating some response from openai" --model=gpt-4
```
...
### deploying/running in the cloud with Modal


it's basically: 

- copy over `.example.env` to `.env` filling in your API key.

We primarily test production deployment with Modal. With Modal we don't experience python dependency hell thanks to using Modal as a [self-provisioning runtime](https://www.google.com/search?q=self+provisioning+runtime).

...
## API
The API we will be building a wrapper around openai's public API endpoints.  It should be able to send prompts and respond with the output. 

Openai require's an api key, which will be in a .env file.  Generate this file, but don't fill in the value for the key, i'll put it in later.
...
{
    "schema_version": "v1",
    "name_for_human": "openai user",
    "name_for_model": "openai",
    "description_for_human": "plugin that makes requests to openai",
    "description_for_model": "plugin that makes requests to openai",
    "auth": {
        "type": "none"
    },
    "api": {
